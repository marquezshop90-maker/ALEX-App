...
import { BookOpen, TrendingUp, Award, Flame, ArrowRight, Star, Clock } from 'lucide-react'
...
export default function Dashboard() {
    ...
    const load = async () => {
      try {
        ...
        const { data: alertData } = await supabase
          .from('system_alerts').select('*').eq('is_active', true)
  ...
  const isInTrial       = trialDaysLeft !== null && trialDaysLeft > 0 && isPremium()
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {isInTrial && (
        <button
          ...
          className="w-full rounded-2xl border border-orange-500/40 bg-orange-500/10 p-4 flex items-center gap-3 hover:bg-orange-500/20 transition-all text-left"
        >
          ...
          <div className="flex-1">
            <p className="font-bold text-white text-sm">
              {isEs
                ? `⏳ Tu prueba Premium termina en ${trialDaysLeft} día${trialDaysLeft === 1 ? '' : 's'}`
                : `⏳ Your Premium trial ends in ${trialDaysLeft} day${trialDaysLeft === 1 ? '' : 's'}`}
            </p>
            <p className="text-orange-300 text-xs mt-0.5">
              {isEs
                ? 'Haz upgrade por solo $45/año para mantener acceso completo →'
                : 'Upgrade for just $45/year to keep full access →'}
            </p>
          </div>
          <Star size={16} className="text-orange-400 flex-shrink-0" fill="currentColor" />
        </button>
      )}
      ...
      {/* Premium CTA for free users only */}
      {isFree && (
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex-1">
              ...
              <p className="font-black text-white text-lg">
                {isEs ? '¿Listo para practicar?' : 'Ready to practice?'}
              </p>
              <p className="text-gray-400 text-sm mt-1">
                {isEs
                  ? 'Flashcards, mini-exámenes, IA y simulados completos por solo $45/año.'
                  : 'Flashcards, mini-exams, AI tutoring & full simulations for just $45/year.'}
              </p>
            </div>
            <Link to="/upgrade" className="btn-primary flex-shrink-0 whitespace-nowrap">
              {isEs ? 'Ver Plan — $45/año' : 'See Plan — $45/yr'}
            </Link>
          </div>
        </div>
      )}
